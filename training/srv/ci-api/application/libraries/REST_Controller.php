...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        if (in_array($referer, $this->config->item('allowed_cors_domains'))) {
    ...
    protected function _log_request($authorized = false)
    {
        ...
        // if (!(substr_count(strtolower(CARRIER_NAME), 'principal', 'vantis') > 0)) {
        //     $organization_check = checkOrganizationPermission($headers);
        // }

        if ( ORG_CHECK == 'TRUE' || ORG_CHECK == '1' || ORG_CHECK == 1) {
            $organization_check = checkOrganizationPermission($headers);
        }

        //Retrive User ID and timezone from access token if the param is passed START
        $user_id = 0;
        $user_timezone = "-08:00";
        $user_converted_time = "";
        $access_token = "";
        $access_token_keywords = array('access-token','access_token');
        foreach( $access_token_keywords as $access_token_keyword )
        {
            if( array_key_exists( $access_token_keyword,$headers ) )
            {
                $access_token = $headers[$access_token_keyword];
            }
        }
        ...
        //Retrive User ID and timezone from access token if the param is passed END
        
        $api_time = function_exists('now') ? now() : time();
        if( $user_timezone != "" )
        {
            $date_from_time = date("Y-m-d H:i:s", $api_time );
            //echo $user_timezone;exit;
            $user_converted_time = getInputTimeBasedOnTimezone( $date_from_time, $user_timezone );
        }
        $user_session_id = 0;       
        if(count($headers) > 0 && isset($headers['sess_id']))       
            $user_session_id = $headers['sess_id'];

        $headers_serialized = serialize($headers);

        $params = $this->_args ? (config_item('rest_logs_json_params') ? json_encode($this->_args) : serialize($this->_args)) : null;
        $platform = 16003;
        
        if (isset($headers["OS"]) && strtolower($headers["OS"]) == "android" || isset($headers["os"]) && strtolower($headers["os"]) == "android") {
            $platform = 16001;
        } else if (isset($headers["OS"]) && strtolower($headers["OS"]) == "ios" || isset($headers["os"]) && strtolower($headers["os"]) == "ios") {
            $platform = 16002;
        }
        ...
        $rest_data = array(
                    ...
                    //'headers_passed' => $headers_serialized,
                    //'params' => $params,
                    'api_key' => isset($this->rest->key) ? $this->rest->key : '',
                    'ip_address' => $this->input->ip_address(),
                    'time' => $api_time,
                    'user_converted_time' => $user_converted_time,
                    'user_active_session_id' => $user_session_id,
                    'authorized' => $authorized,
                    'platform' => $platform
                );
        
        
        $this->rest->db->set("headers_passed",'aes_encrypt("'.addslashes($headers_serialized).'","'.ENCRYPT_SALT.'")', false);
        //unset($rest_data['headers_passed']);

        $this->rest->db->set("params",'aes_encrypt("'.addslashes($params).'","'.ENCRYPT_SALT.'")', false);

        //unset($rest_data['params']);

        $status = $this->rest->db->insert(config_item('rest_logs_table'), $rest_data);

        //echo $this->rest->db->last_query();exit;

        $this->_insert_id = $this->rest->db->insert_id();
        /*if ($user_id > 0) {
            $user_session_data = $this->rest->db->get_where('user_active_logs', array('user_id' => $user_id, 'user_active_session_id' => $user_session_id))->row();        
            if(count($user_session_data) == 0){     
                $status = $this->rest->db->insert('user_active_logs', array(        
                        'user_id' => $user_id,      
                        'user_active_session_id' => $user_session_id,       
                        'start_time' => $date_from_time,        
                        'user_converted_start_time' => $user_converted_time,        
                    ));     
            }       
            else{       
                    $this->rest->db     
                        ->where('user_id', $user_id)        
                        ->where('user_active_session_id', $user_session_id)     
                        ->set('end_time', $date_from_time)      
                        ->set('user_converted_end_time', $user_converted_time)      
                        ->update('user_active_logs');       
            }
        }*/
     ...
     * @param $message
     */
    protected function generateErrorMessage($status, $message) {
        $this->response(array(
            'errors' => array(
                array(
                    'status' => $status,
    ...
    /**This function will be moved to a main controller.
     * For now, this function will be as is
     * @param $exception
     */
    protected function handle_exceptions($exception)
    {
        switch(true){

            case $exception instanceof DatabaseException:
                return $this->generateErrorMessage('Internal Server Error', $exception->getCode());
                break;

            case $exception instanceof MissingPrivilegeException:
                return $this->generateErrorMessage('You are not authorized to access this page', FORBIDDEN);
                break;

            case $exception instanceof ParamsNotFoundException:
            case $exception instanceof InternalServerException:
     ...
     * @param $exception
     */
    protected function handle_exceptions_v2($exception)
    {
        switch(true){

            case $exception instanceof DatabaseException:
                return $this->generateErrorMessageV2('Internal Server Error', $exception->getCode());
                break;

            case $exception instanceof MissingPrivilegeException:
                return $this->generateErrorMessageV2('You are not authorized to access this page', FORBIDDEN);
                break;

            case $exception instanceof ParamsNotFoundException:
            case $exception instanceof InternalServerException:
            case $exception instanceof Exception:
                return $this->generateErrorMessageV2(
                    $exception->getCode(),
                    $exception->getMessage());
     ...
     * @param $message
     */
    protected function generateErrorMessageV2($status, $message) {
        $this->response(array(
            'status' => $status,
            'type' => '',
            'data' => array(),
     ...
     * @param $exception
     */
    protected function handle_exceptions_v3($exception)
    {
        switch(true){

            case $exception instanceof DatabaseException:
                return $this->generateErrorMessageV3($exception->getCode(), 'Internal Server Error');
                break;

            case $exception instanceof MissingPrivilegeException:
                return $this->generateErrorMessageV3(FORBIDDEN, 'You are not authorized to access this page');
                break;

            case $exception instanceof ParamsNotFoundException:
            case $exception instanceof InternalServerException:
            case $exception instanceof Exception:
                return $this->generateErrorMessageV3(
                    $exception->getCode(),
                    $exception->getMessage());
     ...
     * @param $message
     */
    protected function generateErrorMessageV3($status, $message) {
        $this->response(array(
            'type' => '',
            "errors" => array(
                "title" => "",
